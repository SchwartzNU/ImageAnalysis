...
from GUI_helpers import (
    ...
    make_plots_callback,
    prepare_gfp_review_callback,
    accept_gfp_review_callback,
    gfp_threshold_slider_callback,
)
...
DEFAULT_VIEWPORT_WIDTH = 1780
DEFAULT_VIEWPORT_HEIGHT = 1040

OUTER_PAD = 10
COLUMN_GAP = 15
LEFT_COL_WIDTH = 360
SECOND_COL_WIDTH = 390
RIGHT_PANEL_WIDTH = 980

LEFT_COL_X = OUTER_PAD
SECOND_COL_X = LEFT_COL_X + LEFT_COL_WIDTH + COLUMN_GAP
RIGHT_PANEL_X = SECOND_COL_X + SECOND_COL_WIDTH + COLUMN_GAP

TOP_WINDOW_Y = OUTER_PAD
CONTENTS_WINDOW_Y = 195
RIP_WINDOW_Y = 745
BOTTOM_PANEL_Y = 525
...
with dpg.window(tag="left_window", label="Controls", pos=(LEFT_COL_X, TOP_WINDOW_Y), width=LEFT_COL_WIDTH, height=170):
    ...
    dpg.add_text("None", tag="dir_path_repeat", wrap=330)
    ...
    dpg.add_text("Status: Ready", tag="status_text", wrap=330)

with dpg.window(tag="contents_window", label="Folder Contents", pos=(LEFT_COL_X, CONTENTS_WINDOW_Y), width=LEFT_COL_WIDTH, height=535):
...
with dpg.window(tag="rip_panel", label="Rip Panel", pos=(LEFT_COL_X, RIP_WINDOW_Y), width=LEFT_COL_WIDTH, height=125):
...
with dpg.window(tag="analysis_panel", label="Analysis Panel", pos=(SECOND_COL_X, TOP_WINDOW_Y), width=SECOND_COL_WIDTH, height=1005):
    ...
    dpg.add_text("Step 2: Review GFP Detection", color=(200, 200, 0))
    dpg.add_button(label="Prepare GFP Review", tag="prepare_gfp_review_button", width=-1, callback=prepare_gfp_review_callback, enabled=False)
    dpg.add_slider_float(
        label="GFP Threshold",
        tag="gfp_threshold_slider",
        min_value=0.0,
        max_value=1.0,
        default_value=0.0,
        width=180,
        callback=gfp_threshold_slider_callback,
        enabled=False,
    )
    dpg.add_text("Threshold: 0.00", tag="gfp_threshold_value")
    dpg.add_button(label="Accept GFP Detection", tag="accept_gfp_review_button", width=-1, callback=accept_gfp_review_callback, enabled=False)
    dpg.add_text("GFP Status: Prepare GFP review for this file.", tag="gfp_status_text", wrap=360)
    dpg.add_spacer(height=5)
    dpg.add_text("Step 3: Extract Traces", color=(200, 200, 0))
    ...
    with dpg.group(tag="plot_options_group"):
        dpg.add_text("Step 4: Post-Processing Plots", color=(200, 200, 0))
        with dpg.group(horizontal=True):
...
with dpg.window(tag="right_window", label="Image Panel", pos=(RIGHT_PANEL_X, TOP_WINDOW_Y), width=RIGHT_PANEL_WIDTH, height=500):
    with dpg.drawlist(tag="drawlist", width=930, height=420):
        dpg.draw_image("dynamic_texture", (0, 0), (930, 420), tag="main_image_draw")
...
with dpg.window(tag="segmentation_window", label="Segmentation", pos=(RIGHT_PANEL_X, BOTTOM_PANEL_Y), width=RIGHT_PANEL_WIDTH, height=490):
    with dpg.drawlist(tag="segmentation_drawlist", width=930, height=410):
        dpg.draw_image("segmentation_texture", (0, 0), (930, 410), tag="segmentation_image_draw")
...
with dpg.window(tag="gfp_review_window", label="GFP Review", pos=(RIGHT_PANEL_X, BOTTOM_PANEL_Y), width=RIGHT_PANEL_WIDTH, height=490, show=False):
    with dpg.drawlist(tag="gfp_review_drawlist", width=930, height=410):
        dpg.draw_image("gfp_review_texture", (0, 0), (930, 410), tag="gfp_review_image_draw")
